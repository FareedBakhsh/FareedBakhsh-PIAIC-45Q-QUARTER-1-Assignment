...
function make_car(maker, modelName, ...moreInfo) {
    const car = {
        maker: maker,
        modelName: modelName,
    }; // if moreinfor is provided; 
    if (moreInfo.length > 0) {
        //Iterate over moreInfo in pairs and add them to the car object;
        for (let i = 0; i < moreInfo.length; i += 2) {
            const key = moreInfo[i];
            const value = moreInfo[i + 1];
            car[key] = value;
        }
    }
    return car;
...
const myCar = make_car('Toyota', '', 'color', 'blue', 'year', 2022);
