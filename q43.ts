...
//names and one array with the Great added to each magician’s name.
  ...
  function show_magicians(Magicians9: string[]) {
    for (let i = 0; i < Magicians9.length; i++) {
      console.log(Magicians9[i]);
    }
  }
  
  function make_great(Magicians9: string[]) {
  ...
  let Magicians9 = ['Fareed', 'Ihsan', 'Saqlain', 'Sanwal', 'Mohsin', 'Ayaz'];
  
  // Call make_great() with a copy of the magicians array and store the new array
  const greatMagicians = make_great(Magicians9.slice()); // Create a copy using slice()
